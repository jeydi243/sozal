...
const { data: partenaires, error, refresh: refreshPartenaires } = await useAsyncData('partenaires', async () => {
...
const columns: TableColumn<Patient>[] = [
    ...
    },
    {
        ...
        cell: ({ row }) => h(UButton, {
            ...
            variant: 'ghost',
            icon: 'i-lucide-eye',
            to: { name: 'partenaires-id', params: { id: row.original.id } }
        }),
    },
    {
        accessorKey: 'mrn',
            ...
            <template #body>
                <div class="flex flex-wrap items-center justify-between gap-1.5">
                    <UInput :model-value="(table?.tableApi?.getColumn('nom')?.getFilterValue() as string)"
                        class="max-w-sm" icon="i-lucide-search" placeholder="Filter noms..."
